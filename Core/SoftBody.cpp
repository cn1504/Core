...
namespace Core
{
	...
	SoftBody::SoftBody(DynamicsWorld* world, Core::Material* material, btSoftBody* body)
	...
	void SoftBody::RebuildMesh()
	{
		...
		Mesh* newMesh = new Mesh(indices, indexed_vertices, indexed_normals);
		Entity->AddOrReplaceComponent(mesh, newMesh);
		mesh = newMesh;
	}
